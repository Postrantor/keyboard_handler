...
/**
 * @file keyboard_handler_windows_impl.cpp
 *
 * @brief 定义了一个 Windows 平台下的键盘处理实现类 (Defines a keyboard handling implementation class for the Windows platform).
 */

// ch == 0 表示 F1 - F10 键，ch == 0xE0 == 224 表示其他控制键.
// ch == 0 for F1 - F10 keys, ch == 0xE0 == 224 for all other control keys.
//* *INDENT-OFF* */

// 默认静态键映射数组 (Default static key mapping array)
const KeyboardHandlerWindowsImpl::KeyMap KeyboardHandlerWindowsImpl::DEFAULT_STATIC_KEY_MAP[] = {
  ...
  // 向左箭头键 (Left arrow key)
  {KeyCode::CURSOR_LEFT, {0xE0, 75}},
  // 向右箭头键 (Right arrow key)
