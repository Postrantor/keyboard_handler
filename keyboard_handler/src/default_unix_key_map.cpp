...
/// 注意，终端翻译的键码序列可能因不同的终端模拟器而异。请参考
/// https://ftp.metu.edu.tr/pub/mirrors/ftp.x.org/pub/X11R6.8.0/PDF/ctlseqs.pdf。此外根据
/// https://man7.org/linux/man-pages/man5/terminfo.5.html 应用程序可以重新分配一些键码序列。
/// Note that key code sequences translated by the terminal could be different for different terminal
...
{
// 定义光标上移一个单位的字符序列
// Define the character sequence for moving the cursor up by one unit
static constexpr char CURSOR_UP[] = {27, 91, 65, '\0'};

// 定义光标下移一个单位的字符序列
// Define the character sequence for moving the cursor down by one unit
static constexpr char CURSOR_DOWN[] = {27, 91, 66, '\0'};

// 定义光标向右移动一个单位的字符序列
// Define the character sequence for moving the cursor one step to the right
static constexpr char CURSOR_ONE_STEP_RIGHT[] = {27, 91, 67, '\0'};

// 定义光标向左移动一个单位的字符序列
// Define the character sequence for moving the cursor one step to the left
static constexpr char CURSOR_ONE_STEP_LEFT[] = {27, 91, 68, '\0'};

// 定义回车键的字符序列
// Define the character sequence for the Enter key
static constexpr char ENTER[] = {10, '\0'};

// 定义 ESC 键的字符序列
// Define the character sequence for the ESC key
static constexpr char ESC[] = {27, '\0'};

// 定义空格键的字符序列
// Define the character sequence for the Space key
static constexpr char SPACE[] = {32, '\0'};

// 定义双引号的字符序列
// Define the character sequence for the quotation mark
static constexpr char QUOTATION_MARK[] = {34, '\0'};

// 定义反斜杠的字符序列
// Define the character sequence for the backslash
static constexpr char BACK_SLASH[] = {92, '\0'};

// 定义退格键的字符序列
// Define the character sequence for the Backspace key
